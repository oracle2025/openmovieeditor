...
/*
#include "FloatNodeFilterFloat0rFactory.H"
#include "FloatNodeFilterFloat0rFactoryPlugin.H"
#include "Float0rFloatNode.H"
#include "BezierCurveFloatNode.H"
#include "PreviewFloatNode.H"
#include "ImageFloatNode.H"
*/
...
filters* filters_create( int x, int y, int w, int h, IFloatNode* node, string name, int id ) {
	static int id_src = 0;
	filters* p;
	p = new filters;
	p->next = 0;
...
Float0rGraphEditor::Float0rGraphEditor( int x, int y, int w, int h, const char *label )
...
Float0rGraphEditor::~Float0rGraphEditor()
{
	if ( m_filter ) {
...
void Float0rGraphEditor::draw()
{
	...
	for ( filters* i = m_filter->m_filters; i; i = i->next ) {
		...
		for ( int k = 0; i->widgets[k]; k++ ) {
			/*if ( dynamic_cast<BezierCurveFloatNode*>( i->node ) ) {
				i->widgets[k]->resize(x()+i->x+i->w,y()+i->y+k*20,200,200);
			} else if ( dynamic_cast<nle::PreviewFloatNode*>( i->node ) ) {
...
int Float0rGraphEditor::handle( int event )
{
	if ( !m_filter ) {
		...
		case FL_PASTE:
			{
				...
				/*if ( !fl_filename_isdir(fn)) {
					window()->begin();
					nle::ImageFloatNode *in = new nle::ImageFloatNode( fn, m_filter->m_w, m_filter->m_h );
...
void Float0rGraphEditor::addNode( IFloatNodeFilterFactoryPlugin* ffp )
{
	window()->begin();
...
void Float0rGraphEditor::setFilter( nle::FloatNodeFilter* filter )
{
	m_filter = filter;
	assert(m_filter);
	window()->begin();
